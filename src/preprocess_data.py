...
def main(config):
    ...
    if len(unmon_site_data) < num_unmon_sites:
        raise ValueError(
            f"Error: Requested {num_unmon_sites} unmonitored sites in config.json "
            f"(num_unmon_sites_train={num_unmon_sites_train}, num_unmon_sites_test={num_unmon_sites_test}), "
            f"but all_open_world.npz contains only {len(unmon_site_data)} unmonitored traces. "
            f"Please update your config.json to match the available data."
        )
